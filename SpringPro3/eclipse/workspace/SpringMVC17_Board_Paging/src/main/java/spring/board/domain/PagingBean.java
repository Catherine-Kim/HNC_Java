...
public class PagingBean {
	...
	private int numberOfContentPerPage		// 한 페이지에서 보여지는 최대 게시물 수 
								= CommonConstants.CONTENT_NUMBER_PER_PAGE;
	private int numberOfPageGroup			// 한 페이지 그룹 내 페이지 수
								= CommonConstants.PAGEGROUP_NUMBER_PER_PAGE;
	...
	public int getTotalPage(){
		...
		if(num == 0)	// 총 게시물 수가 한 페이지 당 게시물 수의 배수일 때(딱 떨어질 때)
	...
	public boolean isPreviousPageGroup(){
		boolean flag=false;
		if(this.getNowPageGroup() > 1) flag = true;
		return flag;
	}
	...
	public boolean isNextPageGroup(){
		boolean flag=false;
		if(this.getNowPageGroup() < this.getTotalPageGroup()) flag=true;
		return flag;
	}
